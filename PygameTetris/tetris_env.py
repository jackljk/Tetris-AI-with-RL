...
class TetrisEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.observation_space = spaces.Box(low=0, high=np.inf, shape=(40,), dtype=np.float32)
        self.viewer = None
    ...
    def step(self, a):
        ...
        def piece_data_tensor():
            piece_map = {'I0': 0,
                         ...
                         'Z0': 17,
                         'Z1': 18}
            piece_array = np.zeros(19)
            if self.game_state.fallingPiece:
                piece = self.game_state.fallingPiece
                piece_str = piece["shape"]+str(piece["rotation"])
                piece_array[piece_map[piece_str]] = 1
            return piece_array
