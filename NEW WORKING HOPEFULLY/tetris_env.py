...
class GameState:
    ...
    def frame_step(self,input):
        ...
        if not self.isValidPosition(adjY=1):
            ...
            # TODO - Piece lands here start next round
            end_round = True
        else:
        ...
        image_data = pygame.surfarray.array3d(pygame.display.get_surface())
        return image_data, reward, terminal, end_round
    ...
    def simulate_drop(self, board, piece_info):
        """
        Simulate dropping a piece onto the board at a given x_position.
        ...
        :param x_position: The x position to simulate dropping the piece.
        :return: A tuple containing the final board state and the number of lines cleared.
        """
        def _isValidPosition(self, piece_info, adjX=0, adjY=0):
            # Return True if the piece is within the self.board and not colliding
            for x in range(TEMPLATEWIDTH):
                for y in range(TEMPLATEHEIGHT):
                    isAboveBoard = y + piece_info['y'] + adjY < 0
                    if isAboveBoard or PIECES[piece_info['shape']][piece_info['rotation']][y][x] == BLANK:
                        continue
                    if not self.isOnBoard(x + piece_info['x'] + adjX, y + piece_info['y'] + adjY):
                        return False
                    if self.board[x + piece_info['x'] + adjX][y + piece_info['y'] + adjY] != BLANK:
                        return False
            return True
        ...
        for board_x in range(-2, BOARDWIDTH):
            ...
            if _isValidPosition(self, sim_piece):
                ...
                y_position = sim_piece['y']
                # Drop the piece onto the board
                for i in range(1, BOARDHEIGHT):
                    if not _isValidPosition(self, sim_piece, adjY=i):
                        break
                    y_position += 1

                # add the piece to the simulated board
                for x in range(TEMPLATEWIDTH):
                    for y in range(TEMPLATEHEIGHT):
                        if piece[y][x] != BLANK:
                            cell_x = x + board_x
